...
# Loading dataset
products_data = pd.read_csv('C:\\Users\\DELL\\OneDrive\\Máy tính\\BTL_HTTM\\knn_model\\shopping_behavior_updated.csv', index_col='Customer ID')
products_data['Gender'] = products_data['Gender'].replace({'Male': 0.68,
                                                           'Female': 0.32})
products_data['Category'] = products_data['Category'].replace({'Clothing': 0.42,
                                                               'Accessories': 0.30,
                                                               'Footwear': 0.2,
                                                               'Outerwear':0.08})
...
async def suggest_product(suggestions: List[ProductDto]) -> List[ProductResult]:
    ...
    products_suggest = products[products['Category'] == suggestions[0].Category]
    feature_cols= products_suggest.drop(['Item Purchased'], axis=1)
    X = feature_cols
    ...
    for i in range(len(distances.flatten())):
        product_results.append(ProductResult(ItemPurchased=products_suggest['Item Purchased'].iloc[indices.flatten()[i]]))
